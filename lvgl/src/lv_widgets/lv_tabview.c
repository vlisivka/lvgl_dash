...
static lv_res_t lv_tabview_signal(lv_obj_t * tabview, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(indev_type == LV_INDEV_TYPE_KEYPAD ||
           ((indev_type == LV_INDEV_TYPE_ENCODER  || indev_type == LV_INDEV_TYPE_KEYPAD_ENCODER) && lv_group_get_editing(lv_obj_get_group(tabview)))) {
...
#if LV_USE_GROUP
    if(lv_indev_get_type(lv_indev_get_act()) == LV_INDEV_TYPE_ENCODER || lv_indev_get_type(lv_indev_get_act()) == LV_INDEV_TYPE_KEYPAD_ENCODER) {
